package org.opentrackingtools.graph.paths.algorithms.otp.impl;
...
public class MultiDestinationAStar implements
  ...
  public MultiDestinationAStar(Graph graph,
    Set<Edge> endEdges, Coordinate center, double radius,
    Edge start, double distanceUpperLimit) {
  ...
  @Override
  public double
      computeForwardWeight(State s, Vertex target) {
    final Vertex v = s.getVertex();

    final double distance =
        distanceLibrary.fastDistance(v.getCoordinate(),
            center);
    ...
    return (distance - radius)
        / s.getOptions().getCarSpeed();
  }

  @Override
  public double
      computeInitialWeight(State s, Vertex target) {
    return computeForwardWeight(s, target);
  }

  @Override
  public double
      computeReverseWeight(State s, Vertex target) {
    return computeForwardWeight(s, target);
  }
  ...
  public ShortestPathTree getSPT(boolean arriveBy) {
    ...
    final Vertex startVertex =
        arriveBy ? start.getToVertex() : start
            .getFromVertex();
    final String bogusName =
        "bogus" + Thread.currentThread().getId();
    final Vertex bogus =
  ...
  @Override
  public boolean shouldSearchContinue(Vertex origin,
    Vertex target, State current, ShortestPathTree spt,
  ...
  public boolean shouldSkipTraversalResult(Vertex origin,
    Vertex target, State parent, State current,
