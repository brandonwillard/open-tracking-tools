...
import org.openplans.tools.tracking.server.InferenceServiceImpl;
import org.openplans.tools.tracking.server.InferenceServiceImpl.INFO_LEVEL;
import org.openplans.tools.tracking.server.shared.InferenceResultRecord;
...
public class InferenceInstance implements Comparable<InferenceInstance> {
  ...
  private static InferenceGraph inferredGraph = InferenceServiceImpl.getGraph();
  ...
  private boolean shouldProcessUpdate(GpsObservation obs) {
    ...
    if (!isEnabled || InferenceServiceImpl.getInferenceInstance(vehicleId) == null)
      return false;
