...
public class InferenceResultRecord {
  ...
  private static class ResultSet {
    ...
    public double[] getStateCovariance() {
      return ((DenseMatrix)state.getBelief().getCovariance()).convertToVector().getArray();
    }
  ...
  private static ResultSet processVehicleStateResults(VehicleState state) {
    ...
    final PathEdge currentEdge = PathEdge.getEdge(
        Iterables.getLast(path.getEdges()).getInferredEdge(), 0d);
    ...
    for (final PathEdge edge : path.getEdges()) {
       ...
       * step.
       */
      final double edgeMean = edge.getInferredEdge().getVelocityPrecisionDist()
          .getLocation();
    ...
    return new ResultSet(state.clone(), meanCoords, majorAxisCoords, minorAxisCoords,
        pathSegmentIds);
  }
