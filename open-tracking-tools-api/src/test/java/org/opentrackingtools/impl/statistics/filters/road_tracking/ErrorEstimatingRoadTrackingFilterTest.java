...
public class ErrorEstimatingRoadTrackingFilterTest {
  ...
  private void simplePathTest(InferredPath startPath,
    ...
    PathStateBelief currentState =
        PathStateBelief.getPathStateBelief(startPath,
            startState);
    currentState = currentState.getTruncatedPath();
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      if (!presentEdge.isEmptyEdge()) {
        ...
        for (final PathEdge edge : subList) {
          PathEdge newEdge = PathEdge.getEdge(
        ...
        currentEstPath =
            InferredPath.getInferredPath(newEdges,
                direction < 0d);
      } else {
