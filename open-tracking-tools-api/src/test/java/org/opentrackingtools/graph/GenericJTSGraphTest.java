...
public class GenericJTSGraphTest {
  ...
  public void getPaths1() {
    ...
    graphEdges.add(JTSFactoryFinder.getGeometryFactory()
        .createLineString(
    ...
    final Set<LineString> lineStringsToFind =
        new HashSet<LineString>();
    lineStringsToFind.add(JTSFactoryFinder.getGeometryFactory()
        .createLineString(
            new Coordinate[] { new Coordinate(0, 0),
                new Coordinate(1, 0) }));
    lineStringsToFind.add(JTSFactoryFinder.getGeometryFactory()
    ...
    final InferenceGraphSegment startLine =
        Iterables.getOnlyElement(graph.getNearbyEdges(
            graphEdges.get(0).getCoordinate(), 0.5d));
  ...
  public void getPaths2() {
    ...
    graphEdges.add(JTSFactoryFinder.getGeometryFactory()
        .createLineString(
            new Coordinate[] { new Coordinate(2, 0),
                new Coordinate(2, 1) }));
    graphEdges.add(JTSFactoryFinder.getGeometryFactory()
        .createLineString(
    ...
    final VehicleStateInitialParameters parameters =
        new VehicleStateInitialParameters(VectorFactory.getDefault()
            .copyArray(new double[] { 0d, 1d, 0d, 0d }),
            VectorFactory.getDefault().createVector2D(0.5d, 0.5d),
            Integer.MAX_VALUE, VectorFactory.getDefault()
                .createVector1D(1e-4d), Integer.MAX_VALUE,
            VectorFactory.getDefault().createVector2D(1e-4d, 1e-4d),
            Integer.MAX_VALUE, VectorFactory.getDefault()
  ...
  private boolean pathsContainLineStrings(Collection<Path> paths,
    Collection<LineString> lineStrings) {

    // Extract the LineString Geometry from each Path into a Collection we can use
    @SuppressWarnings("unchecked")
    final Collection<LineString> pathLineStrings =
        CollectionUtils.collect(paths, new Transformer() {
    ...
    // in both directions)
    if (pathLineStrings.containsAll(lineStrings)
        && lineStrings.containsAll(pathLineStrings)) {
      return true;
    }
    return false;
  }
  ...
  public void testNoding() {
    ...
    graphEdges.add(new NodedSegmentString(new Coordinate[] {
        new Coordinate(0, 0), new Coordinate(2, 0) }, "1"));
    graphEdges.add(new NodedSegmentString(new Coordinate[] {
        new Coordinate(1, 0), new Coordinate(2, 0) }, "2"));
    graphEdges.add(new NodedSegmentString(new Coordinate[] {
        new Coordinate(2, 0), new Coordinate(1, 0) }, "3"));
    graphEdges.add(new NodedSegmentString(new Coordinate[] {
        new Coordinate(2, 0), new Coordinate(0, 0) }, "4"));
    graphEdges.add(new NodedSegmentString(new Coordinate[] {
        new Coordinate(1, -1), new Coordinate(1, 1) }, "5"));
    ...
    for (final Object obj : noder.getNodedSubstrings()) {
      final NodedSegmentString nss = (NodedSegmentString) obj;
      System.out.println(Arrays.deepToString(nss.getCoordinates())
          + ", " + nss.getData());
