...
public class PathStateTest {
  ...
  private final InferenceGraph graph = Mockito
      .mock(InferenceGraph.class);

  private Path makeTmpPath(boolean isBackward, Coordinate... coords) {
    return TestUtils.makeTmpPath(this.graph, isBackward, coords);
  }
  ...
  public void testBadPathError() {
    ...
    final Geometry e2 =
        JTSFactoryFinder.getGeometryFactory().createLineString(
            new Coordinate[] { new Coordinate(0, 20),
                new Coordinate(0, 30) });
    Mockito.stub(this.graph.edgeHasReverse(e2)).toReturn(false);
    final InferenceGraphEdge ie2_rev =
        new InferenceGraphEdge(e2.reverse(), e2.reverse(), -2,
            this.graph);

    final Geometry e3 =
        JTSFactoryFinder.getGeometryFactory().createLineString(
            new Coordinate[] { new Coordinate(0, 30),
                new Coordinate(0, 40) });
    Mockito.stub(this.graph.edgeHasReverse(e3)).toReturn(false);
    final InferenceGraphEdge ie3_rev =
        new InferenceGraphEdge(e3.reverse(), e3.reverse(), -3,
            this.graph);

    final List<PathEdge> edges2_rev =
        Lists.newArrayList(
            new PathEdge(Iterables.getOnlyElement(ie2_rev
                .getSegments()), 0d, false),
            new PathEdge(Iterables.getOnlyElement(ie3_rev
                .getSegments()), ie2_rev.getLength(), false));
  ...
  public void testDistanceBetween1() {
     ...
     */
    final Path p1 =
        this.makeTmpPath(false, new Coordinate(0, 10),
            new Coordinate(0, 20), new Coordinate(0, 30));
    final PathState x =
        new PathState(p1, VectorFactory.getDenseDefault()
            .createVector2D(5d, 1d));
    final PathState y =
        new PathState(p1, VectorFactory.getDenseDefault()
            .createVector2D(7d, 2d));
  ...
  public void testDistanceBetween10() {
    ...
    final Path path2 =
        this.makeTmpPath(false, new Coordinate(20, 10),
            new Coordinate(10, 10), new Coordinate(0, 10),
            new Coordinate(0, 0));

    final PathState x =
        new PathState(path1, VectorFactory.getDenseDefault()
            .createVector2D(35d, 1d));
    final PathState y =
        new PathState(path2, VectorFactory.getDenseDefault()
            .createVector2D(5d, 1d));
    final Vector diff = y.minus(x);
    ...
    final Vector diff2 = x.minus(y);
    AssertJUnit.assertEquals("rev distance", 50d,
        diff2.getElement(0), this._numericError);
    AssertJUnit.assertEquals("rev velocity", 0d, diff2.getElement(1),
        this._numericError);

    final PathState x2 =
        new PathState(path1, VectorFactory.getDenseDefault()
            .createVector2D(22.5d, 1d));
    final PathState y2 =
        new PathState(path2, VectorFactory.getDenseDefault()
            .createVector2D(5d, 1d));
  ...
  public void testDistanceBetween11() {
    ...
    final Path path1 =
        this.makeTmpPath(false, new Coordinate(0, 0), new Coordinate(
            0, 10));
    ...
    final PathState x =
        new PathState(path2, VectorFactory.getDenseDefault()
            .createVector2D(15d, 1d));
    final PathState y =
        new PathState(path1, VectorFactory.getDenseDefault()
            .createVector2D(8.5d, 1d));
    final Vector diff = y.minus(x);
    ...
    AssertJUnit.assertEquals("rev distance", -13.5d,
        diff2.getElement(0), this._numericError);
    AssertJUnit.assertEquals("rev velocity", -2d,
        diff2.getElement(1), this._numericError);
  }
  ...
  public void testDistanceBetween12() {
    ...
    final PathState startState =
        new PathState(startPath, VectorFactory.getDefault()
            .createVector2D(12d, -1));

    final Path newPath =
        this.makeTmpPath(true, new Coordinate(0, 60), new Coordinate(
            0, 0));

    final PathState result = startState.convertToPath(newPath);

    final PathState stateOnNewPath =
        new PathState(newPath, result.getMotionState());
    final Vector difference = stateOnNewPath.minus(startState);
  ...
  public void testDistanceBetween13() {
    final Path startPath =
        this.makeTmpPath(false, new Coordinate(0, 0), new Coordinate(
            0, 60), new Coordinate(0, 199));

    final PathState startState =
        new PathState(startPath, VectorFactory.getDefault()
            .createVector2D(104d, -1));

    final Path newPath =
        this.makeTmpPath(false, new Coordinate(0, 199),
            new Coordinate(0, 60));
  ...
  public void testDistanceBetween14() {
    final Path startPath =
        this.makeTmpPath(true, new Coordinate(0, 199),
            new Coordinate(0, 60), new Coordinate(0, 0));

    final PathState startState =
        new PathState(startPath, VectorFactory.getDefault()
            .createVector2D(-104d, -2.79));
  ...
  public void testDistanceBetween15() {
    final Path startPath =
        this.makeTmpPath(true, new Coordinate(0, 178.89095954422947),
            new Coordinate(0, 0), new Coordinate(0,
                139.9919743536118d));

    final PathState startState =
        new PathState(startPath, VectorFactory.getDefault()
            .createVector2D(-139.9919743536118d, -3.39));

    final Path newPath =
        this.makeTmpPath(false, new Coordinate(0, 0), new Coordinate(
            0, 178.89095954422947));
  ...
  public void testDistanceBetween16() {
    final Path otherPath =
        this.makeTmpPath(false, new Coordinate(0d, 0d),
            new Coordinate(-85d, 0d), new Coordinate(-150.37d, 0d));

    final PathState otherState =
        new PathState(otherPath, VectorFactory.getDefault()
            .createVector2D(121.15d, 2.59d));

    final Path thisPath =
        this.makeTmpPath(true, new Coordinate(0d, 0d),
            new Coordinate(-85d, 0d), new Coordinate(-150.37d, 0d));
    final PathState thisState =
        new PathState(thisPath, VectorFactory.getDefault()
            .createVector2D(-150.37d, -12.06d));

    final Vector difference = thisState.minus(otherState);

    AssertJUnit.assertEquals("dist", -121d, difference.getElement(0),
        1d);
    AssertJUnit.assertEquals("vel", -14.65d,
        difference.getElement(1), 1d);
  }

  @Test
  public void testDistanceBetween17() {
    final Path startPath =
        this.makeTmpPath(true, new Coordinate(0, -262.21d),
            new Coordinate(0, 0), new Coordinate(0, 792.85d));

    final PathState startState =
        new PathState(startPath, VectorFactory.getDefault()
            .createVector2D(-808.4862233549034, -26.2880263830998));

    final Path newPath =
        this.makeTmpPath(true, new Coordinate(0, 792.85d),
            new Coordinate(0, 0), new Coordinate(0, -262.21d));

    final Vector result =
        startState.convertToPath(newPath).getMotionState();

    final PathState stateOnNewPath = new PathState(newPath, result);
    final Vector difference = stateOnNewPath.minus(startState);
  ...
  public void testDistanceBetween18() {
    final Path otherPath =
        this.makeTmpPath(true, new Coordinate(-90d, 0d),
            new Coordinate(0d, 0d), new Coordinate(0d,
                -(517d - 90d) / 2d), new Coordinate(0d, 0d),
            new Coordinate(-90d, 0d));

    final PathState otherState =
        new PathState(otherPath, VectorFactory.getDefault()
            .createVector2D(-564d, -8d));
  ...
  public void testDistanceBetween19() {
     ...
     */
    final Path thisPath =
        this.makeTmpPath(true, new Coordinate(0d, 607d - 90d),
            new Coordinate(0d, 0d), new Coordinate(-90d, 0d));

    final Vector result =
        otherState.convertToPath(thisPath).getMotionState();

    final PathState stateOnThis = new PathState(thisPath, result);
  ...
  public void testDistanceBetween2() {
    ...
    final PathState x =
        new PathState(p1, VectorFactory.getDenseDefault()
            .createVector2D(5d, 1d));
    final Path p1_neg =
        this.makeTmpPath(true, new Coordinate(0, 10), new Coordinate(
            0, 20), new Coordinate(0, 30));
    final PathState y =
        new PathState(p1_neg, VectorFactory.getDenseDefault()
            .createVector2D(p1_neg.getTotalPathDistance() + 7d, -2d));
    final Vector diff2 = x.minus(y);
  ...
  public void testDistanceBetween20() {
    final Path otherPath =
        this.makeTmpPath(true, new Coordinate(0d, 0d),
            new Coordinate(0d, 179d), new Coordinate(10d, 179d),
            new Coordinate(56d, 179d));

    final PathState otherState =
        new PathState(otherPath, VectorFactory.getDefault()
            .createVector2D(-41d, 4d));
    ...
    final PathState thisState =
        new PathState(thisPath, VectorFactory.getDefault()
            .createVector2D(157d, 7.5d));

    final Vector difference = thisState.minus(otherState);

    AssertJUnit.assertEquals("dist", -37d, difference.getElement(0),
        0d);
    AssertJUnit.assertEquals("vel", 3.5d, difference.getElement(1),
        0d);
  }
  ...
  public void testDistanceBetween21() {
    ...
    final PathState otherState =
        new PathState(otherPath, VectorFactory.getDefault()
            .createVector2D(49d, 1.6d));

    final Path thisPath =
        this.makeTmpPath(false, new Coordinate(0d, 0d),
            new Coordinate(100d, 0d), new Coordinate(100d, 100d),
            new Coordinate(100d, 0d), new Coordinate(0d, 0d));
    final PathState thisState =
        new PathState(thisPath, VectorFactory.getDefault()
    ...
    AssertJUnit.assertEquals("dist", -49d, difference.getElement(0),
        0d);
    AssertJUnit.assertEquals("vel", -1.6d, difference.getElement(1),
        0d);
  }

  /**
   * This test checks that a difference from a state on the first edge of a path
   * that overlaps its first edge, but going the opposite direction, at the end
  ...
  public void testDistanceBetween3() {
    ...
    final PathState x =
        new PathState(p1, VectorFactory.getDenseDefault()
            .createVector2D(12.5d, 1d));
    final Path p2 =
        this.makeTmpPath(false, new Coordinate(0, 20),
            new Coordinate(0, 30), new Coordinate(0, 40));
    final PathState y =
        new PathState(p2, VectorFactory.getDenseDefault()
            .createVector2D(17.5d, -1d));
    final Vector diff = y.minus(x);
  ...
  public void testDistanceBetween4() {
    ...
    final PathState x =
        new PathState(p1, VectorFactory.getDenseDefault()
            .createVector2D(12.5d, 1d));
    final PathState y =
        new PathState(TestUtils.makeTmpPath(this.graph, true,
            new Coordinate(0, 40), new Coordinate(0, 30),
            new Coordinate(0, 20)), VectorFactory.getDenseDefault()
            .createVector2D(-17.5d, -1d));
    final Vector diff = y.minus(x);
    AssertJUnit.assertEquals("distance", -15d, diff.getElement(0),
        this._numericError);
  ...
  public void testDistanceBetween4_2() {
    final PathState x2 =
        new PathState(TestUtils.makeTmpPath(this.graph, false,
            new Coordinate(0, 30), new Coordinate(0, 20),
            new Coordinate(0, 10)), VectorFactory.getDenseDefault()
            .createVector2D(12.5d, 1d));
    final Path p2 =
        this.makeTmpPath(false, new Coordinate(0, 20),
            new Coordinate(0, 30), new Coordinate(0, 40));
  ...
  public void testDistanceBetween6() {
     ...
     */
    final Vector diff2 = x.minus(y);
    AssertJUnit.assertEquals("rev distance", -15d,
        diff2.getElement(0), this._numericError);
    AssertJUnit.assertEquals("rev velocity", 0d, diff2.getElement(1),
        this._numericError);
  }
  ...
  public void testDistanceBetween7() {
    ...
    final Geometry e3 =
        JTSFactoryFinder.getGeometryFactory().createLineString(
            new Coordinate[] { new Coordinate(0, 30),
                new Coordinate(0, 40) });
    Mockito.stub(this.graph.edgeHasReverse(e3)).toReturn(false);
    final InferenceGraphEdge ie3 =
        new InferenceGraphEdge(e3, e3, 3, this.graph);
    ...
    final PathState x =
        new PathState(p1, VectorFactory.getDenseDefault()
            .createVector2D(5d, 1d));
    final PathState y =
        new PathState(new Path(new PathEdge(
            Iterables.getOnlyElement(ie3.getSegments()), 0d, false)),
  ...
  public void testDistanceBetween8() {
    ...
    final InferenceGraphEdge ie3_rev =
        new InferenceGraphEdge(e3.reverse(), e3.reverse(), -3,
            this.graph);
    new LineSegment(ie3_rev.getGeometry().getCoordinates()[0],
        ie3_rev.getGeometry().getCoordinates()[1]);
    final PathState y =
        new PathState(new Path(new PathEdge(
            Iterables.getOnlyElement(ie3_rev.getSegments()), 0d,
            false)), VectorFactory.getDenseDefault().createVector2D(
            5d, -1d));
    final Vector diff = y.minus(x);
    AssertJUnit.assertEquals("distance", -20d, diff.getElement(0),
        this._numericError);
     ...
     */
    final Vector diff2 = x.minus(y);
    AssertJUnit.assertEquals("rev distance", -20d,
        diff2.getElement(0), this._numericError);
    AssertJUnit.assertEquals("rev velocity", 0d, diff2.getElement(1),
        this._numericError);
  }
  ...
  public void testDistanceBetween9() {
    ...
    final PathState toState =
        new PathState(toPath, VectorFactory.getDenseDefault()
            .createVector2D(203.96780646837124, 8.65530011125422));
  ...
  public void testNonPathCombine1() {
    ...
    final PathUtils.PathMergeResults res2 =
        PathUtils.mergePaths(from, 35d, to.reverse(), 5d);
    ...
    final Path path11 =
        this.makeTmpPath(false, new Coordinate(10, 10),
            new Coordinate(20, 10), new Coordinate(20, 20));

    final PathUtils.PathMergeResults res4 =
        PathUtils.mergePaths(path11.getGeometry(), 17.5d, to, 2.5d);
    ...
    final PathUtils.PathMergeResults res5 =
        PathUtils.mergePaths(path11.getGeometry(), 7.5d, to, 2.5d);
    ...
    final PathUtils.PathMergeResults res6 =
        PathUtils.mergePaths(p12, 37.5d, p12, 32.5d);
    ...
    final PathUtils.PathMergeResults res7 =
        PathUtils.mergePaths(p12, 37.5d, p12.reverse(), 32.5d);
