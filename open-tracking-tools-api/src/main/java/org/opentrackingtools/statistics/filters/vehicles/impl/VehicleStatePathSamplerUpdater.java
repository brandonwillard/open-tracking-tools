...
public class VehicleStatePathSamplerUpdater extends
    AbstractVehicleStateUpdater {
  ...
  private static final Logger _log = LoggerFactory
      .getLogger(VehicleStatePathSamplerUpdater.class);
  
  public VehicleStatePathSamplerUpdater(
      GpsObservation obs,
...
//          .getOutgoingTransferableEdges(inferredEdge));
   ...
   * @return
   */
  public PathStateDistribution sampleNextState(VehicleState currentState) {

    final Random rng = this.random;
    rng.setSeed(this.seed);

    PathStateDistribution currentStateBelief = currentState.getBelief();
    OnOffEdgeTransDirMulti edgeTransDist = currentState.getEdgeTransitionDist();
    AbstractRoadTrackingEstimator movementFilter = currentState.getMovementFilter();
      ...
      if (sampledEdge.isNullEdge()) {
        ...
        if (!newEdge.isNullEdge() &&
            !graphExtent.isNull() && !graphExtent.contains(
             GeoUtils.getCoordinates(AbstractRoadTrackingEstimator.getOg().
                 times(projectedMean)))) {
...
//  }
  ...
  public VehicleState update(VehicleState previousState,
    ...
    final AbstractRoadTrackingEstimator predictedFilter =
        previousState.getMovementFilter().clone();

    final Random rng = this.random;
    this.seed = rng.nextLong();

    final PathStateDistribution newPathState =
