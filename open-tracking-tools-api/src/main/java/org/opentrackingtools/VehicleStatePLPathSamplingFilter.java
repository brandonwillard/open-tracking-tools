...
//       Iterables.getLast(priorPredictivePathStateDist.getPathState().getPath().getPathEdges())));
    ...
    if (!(posteriorPathStateDist.getPathState().isOnRoad() &&
      ...
      newObsStateSample = MotionStateEstimatorPredictor.getOg().times(
        modelCovarianceEstimator.getNewPosteriorStateSample().getGroundDistribution().getMean());
    } else {
      newObsStateSample = MotionStateEstimatorPredictor.getOg().times(
          posteriorPathStateDist.getGroundDistribution().getMean());
    }
    ...
    if (state.getObservationCovarianceParam().getParameterPrior().getDegreesOfFreedom()
      ...
      updatedState.getObservationCovarianceParam().setConditionalDistribution(
          new MultivariateGaussian(VectorFactory.getDefault().createVector(obsCovSample.getNumColumns()), 
              obsCovSample));
    }
