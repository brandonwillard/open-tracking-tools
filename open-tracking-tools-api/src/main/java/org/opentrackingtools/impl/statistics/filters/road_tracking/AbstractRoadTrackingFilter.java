package org.opentrackingtools.impl.statistics.filters.road_tracking;
...
import org.opentrackingtools.impl.statistics.filters.AdjKalmanFilter;
...
public abstract class AbstractRoadTrackingFilter<T extends AbstractRoadTrackingFilter<T>>
    ...
    if (belief.isOnRoad()) {
      ...
      final MultivariateGaussian obsProj =
          AbstractRoadTrackingFilter.getRoadObservation(
              observation, this.obsCovar, belief.getPath(),
              edge);
      ...
      final PathStateBelief tmpBelief =
          PathStateBelief.getPathStateBelief(
              belief.getPath(), updatedBelief);

      final List<PathEdge> edges = Lists.newArrayList();
      for (final PathEdge pedge : belief.getPath()
          .getEdges()) {
        edges.add(pedge);
      ...
      result =
          PathStateBelief.getPathStateBelief(InferredPath
              .getInferredPath(edges, belief.getPath()
                  .getIsBackward()), updatedBelief);
  ...
  public PathStateBelief predict(
    PathStateBelief currentBelief, InferredPath newPath) {
    ...
    if (newPath.isEmptyPath()) {
      ...
      if (!currentBelief.isOnRoad()) {
        ...
        convertToRoadBelief(newBelief, newPath,
            Iterables.getFirst(newPath.getEdges(), null),
            true);
      } else {
    ...
    return PathStateBelief.getPathStateBelief(newPath,
        newBelief);
  }
  ...
  public static MultivariateGaussian
    ...
    convertToRoadBelief(
        ...
        InferredPath.getInferredPath(PathEdge.getEdge(
            edge.getInferredEdge(), 0d, edge.isBackward())),
        true);
...
//      projMean.setElement(1, projVelocity);
  ...
  public abstract void update(VehicleState state,
    Observation obs, PathStateBelief updatedBelief,
