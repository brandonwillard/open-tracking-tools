...
public class VehicleTrackingPathSamplerFilterUpdater extends
  ...
  private Collection<? extends InferredEdge>
      getTransferableEdges(InferredEdge currentEdge,
        Vector newState) {
    /*
  ...
  public PathState sampleNextState(
    ...
    PathEdge currentEdge =
        PathEdge.getEdge(initialState.getEdge()
            .getInferredEdge(), 0d, initialState.getPath()
            .getIsBackward());
    ...
    while (totalDistToTravel == null
      ...
      if (currentEdge.isEmptyEdge()) {
        ...
        for (final StreetEdge edge : this.inferredGraph
          ...
          transferEdges.add(this.inferredGraph
...
//          transferEdges.addAll(equivEdges);
          transferEdges.add(initialState.getEdge().getInferredEdge());
          ...
          if (transferEdges.isEmpty()) {
             ...
             */
            newState.setElement(0, initialLocation
                + distTraveled);
      ...
      final InferredEdge sampledEdge =
          edgeTransDist.sample(
              rng,
              transferEdges,
              currentEdge == null ? initialState.getEdge()
      ...
      if (sampledEdge == InferredEdge.getEmptyEdge()) {
        ...
        if (!this.inferredGraph.getBaseGraph().getExtent()
          ...
          final Vector oldLoc =
              AbstractRoadTrackingFilter.getOg().times(
                  initialState.getGroundState());
          final Polygon extent =
              JTS.toGeometry(this.inferredGraph
                  .getBaseGraph().getExtent());
      ...
      if (totalDistToTravel == null) {
        ...
        if (newState.getDimensionality() == 4) {
          final InferredPath edgePath =
              InferredPath.getInferredPath(PathEdge
                  .getEdge(sampledEdge, 0d, false));
          newState =
              AbstractRoadTrackingFilter
                  .convertToRoadState(newState, edgePath,
                      true).getLocalState();
        } else {
          /*
          ...
          newState =
              edgePath
                  .getStateOnPath(PathState.getPathState(
                      InferredPath
                          .getInferredPath(PathEdge.getEdge(
                              currentEdge.getInferredEdge(),
                              0d,
        ...
        totalDistToTravel =
            newState.getElement(0) - initialLocation;
         ...
         */
        currentEdge =
            PathEdge.getEdge(sampledEdge, 0d,
                newLocation < 0d);
        ...
        if (sampledEdge == null) {
           ...
           */
          newState.setElement(0,
              currentEdge.getDistToStartOfEdge()
                  + direction
                  * currentEdge.getInferredEdge()
                      .getLength());
          newState.setElement(1, 0d);
    ...
    if (newState.getDimensionality() == 2) {
      ...
      result =
          Preconditions.checkNotNull(newPath
              .getCheckedStateOnPath(newState,
                  AbstractRoadTrackingFilter
                      .getEdgeLengthErrorTolerance()));
    } else {
      final InferredPath newPath =
          InferredPath.getEmptyPath();
  ...
  public VehicleState update(VehicleState previousState,
    ...
    final PathStateBelief newStateBelief =
        PathStateBelief.getPathStateBelief(
            newPathState.getPath(), newBelief);

    final VehicleState newState =
        new VehicleState(this.inferredGraph, obs,
            predictedFilter, newStateBelief, newTransDist,
            previousState);
