package org.opentrackingtools.impl.graph;
...
public class InferredEdge implements
   ...
   */
  private final static InferredEdge emptyEdge =
      new InferredEdge();
  ...
  public InferredEdge(@Nonnull Edge edge,
    @Nonnull Integer edgeId, @Nonnull OtpGraph graph) {
    ...
    final Collection<Edge> baseEdges =
        graph.getGeomBaseEdgeMap().get(edge.getGeometry());
    boolean hasReverseTmp = false;
    for (final Edge bEdge : baseEdges) {
      if (bEdge.getGeometry().reverse().equalsExact(edge.getGeometry()))
        hasReverseTmp = true;
    }
    this.hasReverse = hasReverseTmp;
  ...
  public List<InferredEdge> getIncomingTransferableEdges() {
    ...
    for (final Edge edge : OtpGraph
        .filterForStreetEdges(this.startVertex
            .getIncoming())) {
  ...
  public List<InferredEdge> getOutgoingTransferableEdges() {
    final List<InferredEdge> result = Lists.newArrayList();
    for (final Edge edge : OtpGraph
        .filterForStreetEdges(this.endVertex
