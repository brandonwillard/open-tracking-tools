...
public abstract class AbstractPathState extends
  ...
  public Vector minus(AbstractPathState otherState) {
    if (this.isOnRoad() && otherState.isOnRoad()) {
      ...
      final PathEdge otherFirstEdge =
          Iterables.getFirst(otherState.path.getEdges(),
              null);
      final PathEdge otherLastEdge =
          Iterables.getLast(otherState.path.getEdges(),
              null);
      final Geometry otherFirstActualGeom =
          otherState.path.getIsBackward() ? otherFirstEdge
      ...
      if (otherState.path.getEdges().size() > 1) {
        otherLastActualGeom =
            otherState.path.getIsBackward() ? otherLastEdge
      ...
      if (otherLastActualGeom
        ...
        if (this.path.getIsBackward() == otherState.path
            .getIsBackward()) {
        ...
        distanceMax =
            Math.max(Math.abs(otherState.path
  ...
  private static Vector headToTailRevDiff(
    ...
    final double otherDir =
        otherState.path.getIsBackward() ? -1d : 1d;
    final Vector otherStateVec =
        useRaw ? otherState.getRawState() : otherState
            .getGlobalState();
    final Vector thisStateVec =
