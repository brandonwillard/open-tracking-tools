...
import org.opentrackingtools.graph_builder.PlainStreetEdgeWithOSMData;
import org.opentrackingtools.graph_builder.TurnVertexWithOSMData;
import org.opentrackingtools.impl.VehicleState;
import org.opentrackingtools.impl.graph.BaseGraph;
...
import org.opentrackingtools.impl.graph.paths.algorithms.MultiDestinationAStar;
...
public class OtpGraph {
  ...
  private Set<InferredPath> computePaths(PathKey key) {
    ...
    for (final Object obj : getNearbyEdges(toCoord,
        obsStdDevDistance)) {
...
//        }
   ...
   * @return
   */
  public Set<StreetEdge> getNearbyEdges(Coordinate loc,
    double radius) {
  ...
  public List<StreetEdge> getNearbyEdges(
    ...
    final List<StreetEdge> streetEdges =
        Lists.newArrayList();
    for (final Object obj : baseEdgeIndex.query(toEnv)) {
  ...
  public Set<StreetEdge> getNearbyEdges(Vector loc,
    double radius) {
    Preconditions
        .checkArgument(loc.getDimensionality() == 2);
    return getNearbyEdges(GeoUtils.makeCoordinate(loc),
        radius);
  }
  ...
  private PathEdge getValidPathEdge(Edge originalEdge,
    ...
    if (OtpGraph.isStreetEdge(edge)
      ...
      return PathEdge.getEdge(this.getInferredEdge(edge),
  ...
  public Envelope getTurnGraphExtent() {
