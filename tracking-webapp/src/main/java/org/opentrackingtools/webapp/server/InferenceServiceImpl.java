...
import org.opentrackingtools.webapp.client.InferenceService;
 ...
 * 
 */
public class InferenceServiceImpl extends RemoteServiceServlet implements
		InferenceService {
	...
	private static Map<String, Class<? extends VehicleTrackingFilter>> filtersMap = Maps
			.newHashMap();
	static {
		filtersMap.put(VTErrorEstimatingPLFilter.class.getName(),
				VTErrorEstimatingPLFilter.class);
		filtersMap.put(VehicleTrackingPLFilter.class.getName(),
				VehicleTrackingPLFilter.class);
		filtersMap.put(VehicleTrackingBootstrapFilter.class.getName(),
				VehicleTrackingBootstrapFilter.class);
	}
	...
	private static VehicleStateInitialParameters defaultVehicleStateInitialParams = new VehicleStateInitialParameters(
			VectorFactory.getDefault().createVector2D(100d, 100d), 20,
			VectorFactory.getDefault().createVector2D(0.000625, 0.000625), 20,
			VectorFactory.getDefault().createVector2D(0.000625, 0.000625), 20,
			VectorFactory.getDefault().createVector2D(5d, 95d), VectorFactory
					.getDefault().createVector2D(95d, 5d),
			VehicleTrackingPLFilter.class.getName(), 25, 30, 0l);
	...
	public static final String defaultFilterName = Iterables.getFirst(
			filtersMap.keySet(), null);
	...
	public static VehicleStateInitialParameters getDefaultVehicleStateInitialParams() {
	...
	public static InferenceInstance getOrCreateInferenceInstance(
			String vehicleId, VehicleStateInitialParameters initialParameters,
			VehicleStateInitialParameters simParameters, INFO_LEVEL infoLevel) {

		InferenceInstance ie = vehicleToInstance.get(vehicleId);

		if (ie == null) {
			ie = new InferenceInstance(vehicleId, simParameters, infoLevel,
					initialParameters);
	...
	public static boolean processRecord(GpsObservation observation) {
		...
		executor.execute(new UpdateRunnable(Collections
				.singletonList(observation), ie));

		return true;
	}

	public static void processRecords(List<GpsObservation> observations,
			VehicleStateInitialParameters initialParameters,
			String filterTypeName, INFO_LEVEL level)
			throws InterruptedException {

		final Multimap<InferenceInstance, GpsObservation> instanceToObs = TreeMultimap
				.create();

		for (final GpsObservation obs : observations) {
			final InferenceInstance ie = getOrCreateInferenceInstance(
					obs.getSourceId(), initialParameters, null, level);
		...
		for (InferenceInstance instance : instanceToObs.keySet()) {
			tasks.add(Executors.callable(new UpdateRunnable(Lists
					.newArrayList(instanceToObs.get(instance)), instance)));
		}
	...
	public static void setDefaultVehicleStateInitialParams(
		...
		InferenceServiceImpl.defaultVehicleStateInitialParams = defaultVehicleStateInitialParams;
	}
