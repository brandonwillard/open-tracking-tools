package org.opentrackingtools.webapp.server;
...
import org.opentrackingtools.webapp.client.TraceUploadService;
import org.opentrackingtools.webapp.server.InferenceServiceImpl.INFO_LEVEL;
import org.opentrackingtools.webapp.server.model.InferenceInstance;
import org.opentrackingtools.webapp.server.model.ObservationFactory;
 ...
 */
public class TraceUploadServiceImpl extends RemoteServiceServlet implements
		TraceUploadService {
	...
	public static class TraceParameters {
		...
		public VehicleStateInitialParameters getVehicleStateInitialParams() {
		...
		public int hashCode() {
			...
			result = prime
					* result
					+ ((filterTypeName == null) ? 0 : filterTypeName.hashCode());
			result = prime
					* result
					+ ((vehicleStateParams == null) ? 0 : vehicleStateParams
							.hashCode());
			return result;
		}
			...
			try {
				...
				while (iter.hasNext()) {
					...
					if (uploadedFileItem.isFormField()) {
						...
						if (uploadedFileItem.getFieldName().equalsIgnoreCase(
								"debugEnabled")) {
							debugEnabled = Boolean
									.parseBoolean(uploadedFileItem.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("obsCov")) {
							obsCov = parseVector(uploadedFileItem.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("obsCovDof")) {
							obsCovDof = Integer.parseInt(uploadedFileItem
									.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("onRoadStateCov")) {
							onRoadStateCov = parseVector(uploadedFileItem
									.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("onRoadCovDof")) {
							onRoadCovDof = Integer.parseInt(uploadedFileItem
									.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("offRoadStateCov")) {
							offRoadStateCov = parseVector(uploadedFileItem
									.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("offRoadCovDof")) {
							offRoadCovDof = Integer.parseInt(uploadedFileItem
									.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("offProbs")) {
							offProbs = parseVector(uploadedFileItem.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("onProbs")) {
							onProbs = parseVector(uploadedFileItem.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("filterTypeName")) {
							filterTypeName = uploadedFileItem.getString();
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("numParticles")) {
							numParticles = Integer.parseInt(uploadedFileItem
									.getString());
						} else if (uploadedFileItem.getFieldName()
								.equalsIgnoreCase("initialObsFreq")) {
							initialObsFreq = Integer.parseInt(uploadedFileItem
									.getString());
						} else if (uploadedFileItem.getFieldName()
				...
				VehicleStateInitialParameters vehicleStateParams = new VehicleStateInitialParameters(
						obsCov, obsCovDof, onRoadStateCov, onRoadCovDof,
						offRoadStateCov, offRoadCovDof, offProbs, onProbs,
						filterTypeName, numParticles, initialObsFreq, seed);
				TraceParameters parameters = new TraceParameters(file,
						vehicleStateParams, debugEnabled);
	...
	final private void readCsv(TraceParameters traceParams) throws IOException,
		...
		try {
			do {
				try {
					...
					final GpsObservation obs = ObservationFactory
							.createObservation(vehicleId, line[6], lat, lng,
									line[7], null, null);
		...
		InferenceServiceImpl.processRecords(observationFactories,
				traceParams.getVehicleStateInitialParams(),
				traceParams.getFilterTypeName(), level);
		InferenceServiceImpl.getExecutor()
				.awaitTermination(5, TimeUnit.SECONDS);

		log.info("finished processing " + fileName);

		for (final String vehicleId : vehicleIds) {
			final InferenceInstance ie = InferenceServiceImpl
